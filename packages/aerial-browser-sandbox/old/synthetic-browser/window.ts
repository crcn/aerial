...
import { getSyntheticLocationClass, createProxyLocationHandler } from "./location";
import { 
  DOMEventTypes,
  IDOMEventEmitter,
  SyntheticDOMEvent,
...
const SyntheticLocation = getSyntheticLocationClass(global);

export interface SyntheticWindowInterface {

}

export class SyntheticWindow extends Observable implements SyntheticWindowInterface {
  ...
  constructor(url?: string, document?: SyntheticDocument) {
    super();

    this.location = new Proxy(new SyntheticLocation(url), createProxyLocationHandler(() => {
      // TODO
    }))

    this.document = new SyntheticDocument(HTML_XMLNS);
