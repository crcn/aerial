...
var RemoteBrowserDocumentMessage = (function (_super) {
    ...
    var RemoteBrowserDocumentMessage = RemoteBrowserDocumentMessage_1 = function RemoteBrowserDocumentMessage(type, data) {
...
var RemoteSyntheticBrowser = (function (_super) {
    ...
    function RemoteSyntheticBrowser(kernel, renderer, parent) {
        ...
        _this.sendDiffs = lodash_1.debounce(function () {
            ...
            _this._mutations = [];
            _this._writer.write(aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.DOCUMENT_DIFF, mutations)));
        }, 100);
    ...
    RemoteSyntheticBrowser.prototype.onDocumentEvent = function (event) {
        ...
        if (event.target && event.target.clone) {
            ...
            this._writer.write(aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.DOM_EVENT, [dom_1.getNodePath(event.target), event])));
        }
    };
    __decorate([
    ...
    RemoteBrowserService.prototype[messages_1.OpenRemoteBrowserRequest.OPEN_REMOTE_BROWSER] = function (event) {
        ...
        return new mesh_1.DuplexStream(function (input, output) {
            ...
            if (browser.logs.length) {
                writer.write({ payload: aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.VM_LOG, browser.logs.map(function (log) { return [log.level, log.text]; }))) });
            }
            var changeWatcher = new aerial_sandbox_1.SyntheticObjectChangeWatcher(function (mutations) { return __awaiter(_this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0:
                            logger.info("Sending diffs: <<", mutations.map(function (event) { return event.type; }).join(", "));
                            browser.sandbox.pause();
                            return [4 /*yield*/, writer.write({ payload: aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.DOCUMENT_DIFF, mutations)) })];
                        case 1:
                            _a.sent();
                ...
                editor = new aerial_sandbox_1.SyntheticObjectTreeEditor(clone);
                logger.info("Sending <<new document");
                writer.write({ payload: aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.NEW_DOCUMENT, clone)) });
            });
            if (browser.document) {
            ...
            input.pipeTo(new mesh_1.WritableStream({
                write: function (payload) {
                    var message = aerial_common_1.deserialize(payload, _this.kernel);
            ...
            var onStatusChange = function (status) {
                ...
                writer.write({ payload: aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.STATUS_CHANGE, status)) });
            };
            var browserObserver = new mesh_1.CallbackBus(function (event) {
                if (event.type === aerial_common_1.LogEvent.LOG) {
                    var logEvent = event;
                    console.log("EMITTING LOG", logEvent.text);
                    writer.write({ payload: aerial_common_1.serialize(new RemoteBrowserDocumentMessage(RemoteBrowserDocumentMessage.VM_LOG, [[logEvent.level, logEvent.text]])) });
                }
            });
            browser.observe(browserObserver);
            var watcher = aerial_common_1.watchProperty(browser, "status", onStatusChange);
...
}(aerial_common_1.BaseApplicationService));
