...
var SyntheticDocumentSerializer = (function () {
    ...
    SyntheticDocumentSerializer.prototype.serialize = function (document) {
        return [
            ...
            [].concat(document.styleSheets).map(aerial_common_1.serialize),
            document.childNodes.map(aerial_common_1.serialize),
        ];
    };
    SyntheticDocumentSerializer.prototype.deserialize = function (_a, kernel) {
        ...
        (_b = document.styleSheets).push.apply(_b, styleSheets.map(function (raw) { return aerial_common_1.deserialize(raw, kernel); }));
        for (var i = 0, n = childNodes.length; i < n; i++) {
            document.appendChild(aerial_common_1.deserialize(childNodes[i], kernel));
        }
        return document;
        var _b;
    };
...
var SyntheticDocumentEdit = (function (_super) {
    ...
    SyntheticDocumentEdit.prototype.addStyleSheet = function (stylesheet, index) {
        return this.addChange(new aerial_common_1.InsertChildMutation(SyntheticDocumentMutationTypes.ADD_DOCUMENT_STYLE_SHEET_EDIT, this.target, stylesheet, index));
    };
    SyntheticDocumentEdit.prototype.removeStyleSheet = function (stylesheet) {
        return this.addChange(new aerial_common_1.RemoveChildMutation(SyntheticDocumentMutationTypes.REMOVE_DOCUMENT_STYLE_SHEET_EDIT, this.target, stylesheet, this.target.styleSheets.indexOf(stylesheet)));
    };
    SyntheticDocumentEdit.prototype.moveStyleSheet = function (stylesheet, index) {
        return this.addChange(new aerial_common_1.MoveChildMutation(SyntheticDocumentMutationTypes.MOVE_DOCUMENT_STYLE_SHEET_EDIT, this.target, stylesheet, this.target.styleSheets.indexOf(stylesheet), index));
    };
    SyntheticDocumentEdit.prototype.addDiff = function (newDocument) {
        var _this = this;
        aerial_common_1.diffArray(this.target.styleSheets, newDocument.styleSheets, function (oldStyleSheet, newStyleSheet) {
    ...
    SyntheticDocumentEdit = __decorate([
        aerial_common_1.serializable("SyntheticDocumentEdit", {
            ...
            deserialize: function (_a, kernel, ctor) {
                var mutations = _a.mutations;
                var edit = new ctor();
                (_b = edit.mutations).push.apply(_b, mutations.map(function (mutation) { return aerial_common_1.deserialize(mutation, kernel); }));
                return edit;
                var _b;
            }
        })
...
var SyntheticDocument = (function (_super) {
    __extends(SyntheticDocument, _super);
    var SyntheticDocument = SyntheticDocument_1 = 
    ...
    function SyntheticDocument(defaultNamespaceURI, implementation) {
        ...
        _this.styleSheets = aerial_common_1.ObservableCollection.create();
        _this.styleSheets.observe(new mesh_1.CallbackBus(_this.onStyleSheetsEvent.bind(_this)));
    ...
    SyntheticDocument.prototype.onStyleSheetsEvent = function (_a) {
        ...
        if (!mutation)
            return;
        if (mutation.type === aerial_common_1.ArrayMutation.ARRAY_DIFF) {
            mutation.accept({
                visitUpdate: function (_a) {
                    var newValue = _a.newValue, index = _a.index, patchedOldIndex = _a.patchedOldIndex;
                    if (index !== patchedOldIndex) {
                        _this.notify(new aerial_common_1.MoveChildMutation(SyntheticDocumentMutationTypes.MOVE_DOCUMENT_STYLE_SHEET_EDIT, _this, newValue, patchedOldIndex, index).toEvent());
                    }
                },
                visitInsert: function (_a) {
                    ...
                    _this.notify(new aerial_common_1.InsertChildMutation(SyntheticDocumentMutationTypes.ADD_DOCUMENT_STYLE_SHEET_EDIT, _this, value, index).toEvent());
                },
                visitRemove: function (_a) {
                    var value = _a.value, index = _a.index;
                    value.$ownerNode = undefined;
                    _this.notify(new aerial_common_1.RemoveChildMutation(SyntheticDocumentMutationTypes.REMOVE_DOCUMENT_STYLE_SHEET_EDIT, _this, value, index).toEvent());
                }
            });
        }
    };
    SyntheticDocument = SyntheticDocument_1 = __decorate([
        aerial_common_1.serializable("SyntheticDocument", new markup_1.SyntheticDOMNodeSerializer(new SyntheticDocumentSerializer()))
    ], SyntheticDocument);
