...
export function calculateAbsoluteBounds(node: HTMLElement) {
  let rect: BoundingRect = calculateUntransformedBoundingRect(node);

  return rect;
}

function calculateTransforms(node: HTMLElement) {
  const computedStyle: any = calculateCSSMeasurments(window.getComputedStyle(node));
