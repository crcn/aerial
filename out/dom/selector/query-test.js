...
describe(__filename + "#", function () {
    [
        ...
        var selector = _a[0], a = _a[1], b = _a[2];
        it("selector " + selector + " for " + a + " equals " + b, function () {
            var document = new __1.SyntheticWindow().document;
            document.registerElement("div", __1.SyntheticHTMLElement);
            document.registerElement("span", __1.SyntheticHTMLElement);
            document.removeAllChildren();
            document.appendChild(__1.evaluateMarkup(parse5.parseFragment(a, { locationInfo: true }), document));
            var nodes = document.querySelectorAll(selector);
            chai_1.expect(nodes.join("")).to.equal(b);
        });
    });
});
